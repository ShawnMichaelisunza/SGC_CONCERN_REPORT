...
@section('content')
    <div class="flex h-screen bg-white">
        ...
        <div class="flex flex-col flex-1 overflow-y-auto">
            ...
            <div class="flex justify-between py-7 px-9">
                <div>
                    <h2 class="text-3xl uppercase">{{ $user->name }}</h2>
                    <h2 class="text-md text-gray-600 font-semibold">{{ $user->email }}</h2>

                </div>
                <div>
                    <h2 class="text-end text-md text-gray-600 font-semibold">{{ $user->company_name }}</h2>
                    <h2 class="text-end text-md text-gray-600 font-semibold">{{ $user->dept }}</h2>
                    <h2 class="text-end text-lg text-red-600 font-semibold">{{ $user->emp_no }}</h2>
                </div>
            </div>
                    {{-- table --}}
                    <div class="px-4 py-5">
                        ...
                        <div class="flex justify-end gap-3 p-2 ">
                            <a href="{{ route('user.profile', encrypt(auth()->user()->id)) }}" class="text-xs border bg-red-700 border-red-700 hover:bg-red-600 text-white font-semibold py-1 rounded-sm px-4">ALL</a>
                            <a href="{{ route('user.pending', encrypt(auth()->user()->id)) }}" class="text-xs border bg-red-700 border-red-700 hover:bg-red-600 text-white font-semibold py-1 rounded-sm px-4">PENDING</a>
                            <a href="{{ route('user.process', encrypt(auth()->user()->id)) }}" class="text-xs border bg-red-700 border-red-700 hover:bg-red-600 text-white font-semibold py-1 rounded-sm px-4">PROCESSING</a>
                            <a href="{{ route('user.completed', encrypt(auth()->user()->id)) }}" class="text-xs border bg-red-700 border-red-700 hover:bg-red-600 text-white font-semibold py-1 rounded-sm px-4">COMPLETED</a>
                            <a href="{{ route('user.canceled', encrypt(auth()->user()->id)) }}" class="text-xs border bg-red-700 border-red-700 hover:bg-red-600 text-white font-semibold py-1 rounded-sm px-4">CANCELED</a>
                                ...
                                <tbody class="text-gray-600 text-sm">
                                    @foreach ($reports as $report)
                                        <tr class="border-b border-gray-200 hover:bg-gray-200">
                                            ...
                                            <td class="py-3 px-6 text-left">{{ $report->company_name }}</td>
