...
<div class="hidden peer-checked:flex md:flex flex-col w-64 bg-red-800 transition-all duration-300 ease-in-out">
    ...
    <div class="flex flex-col flex-1 overflow-y-auto">
        <nav class="flex-1 px-2 py-4 bg-gray-300">
            <a href="{{ route('dashboard') }}" class="flex items-center px-4 py-2 text-gray-800 hover:text-gray-100 hover:bg-red-700 group">
                <i class="fa-solid fa-list h-7 w-7 mr-2 pt-2"></i>
                DASHBOARD
            </a>


            <a href="{{ route('employee_pending') }}"
                class="flex items-center px-4 py-2 text-gray-800 hover:text-gray-100 hover:bg-red-700 group">
                <i class="fa-solid fa-spinner  h-7 w-7 mr-2 pt-2"></i>
                PENDING REPORT
            </a>

            <a href="{{ route('employee_processed') }}"
                class="flex items-center px-4 py-2 text-gray-800 hover:text-gray-100 hover:bg-red-700 group">
                <i class="fa-solid fa-stopwatch h-7 w-7 mr-2 pt-2"></i>
                PROCESSING REPORT
            </a>

            <a href="{{ route('employee_completed') }}"
                class="flex items-center px-4 py-2 text-gray-800 hover:text-gray-100 hover:bg-red-700 group">
                <i class="fa-solid fa-star  h-7 w-7 mr-2 pt-2"></i>
                COMPLETED REPORT
            </a>

            <a href="{{ route('employee_disapproved') }}"
                class="flex items-center px-4 py-2 text-gray-800 hover:text-gray-100 hover:bg-red-700 group">
            ...
            <a href="{{ route('logout') }}"
                class="flex items-center px-4 py-2 my-8 text-gray-800 hover:text-gray-100 hover:bg-red-700 group">
